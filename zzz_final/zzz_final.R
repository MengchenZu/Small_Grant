...
# if we generate mallot from the beginning, run codes from here
data_dir <- file.path("data")

# load metadata
metadata_folder <- file.path("metadata")
myfiles <- list.files(path = metadata_folder, pattern = ".tsv", full.names = TRUE)
meta <- NULL
for(metadata_file in myfiles) {
  meta_part <- my_read_dfr_metadata(metadata_file)
  meta <- rbind(meta, meta_part)
}
...
# if we want to load the mallet directly, run the code from here
# metadata_folder <- file.path("metadata")
# m <- my_load_mallet_model_directory("modeling_results", metadata_folder = metadata_folder)
